...
public class EscapeRoom extends PApplet {
  ...
  private PImage backgroundImage; // room background image
  private ArrayList<Thing> allThings = new ArrayList<>(); // list of items in the room

  /**
   * Sets the settings for the PApplet window
   */
  public void settings() {
    size(800,600); // sets width and height
  }
  ...
  /**
   * Draws the background image, then adds and removes thing objects.
   */
  public void draw() {
    image(backgroundImage, 0.0f, 0.0f); // loads the background image
    for (int i = 0; i < allThings.size(); i++) { // iterates through the array of thing objects
      Action action =  allThings.get(i).update(); // sets the action
      if (action != null) {
        action.act(allThings); // performs the action
      }
      if(!allThings.get(i).isActive()){
        allThings.remove(i); // removes the object if it is no longer active
        i--;
      }
    }
  }
